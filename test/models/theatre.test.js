...
describe('Theatre Model', function() {
  ...
  describe('.find()', function() {
    ...
    it('should throw an error if the given theatre cannot be found', function() {
      return Theatre.find(null).then(response => expect(response).to.not.be.ok).catch(error => expect(error).to.be.ok)
    })
  })
  ...
  describe('#save()', function() {
    it('should create a new record if there is no id stored in the instance', async function() {
      ...
      await theatre.save()
      const row = await global.knex('theatres').limit(1).orderBy('id', 'desc').first()
    ...
    it('should throw an error if any of the values are missing on creation', function() {
      ...
      return theatre.save().then(response => expect(response).to.not.be.ok).catch(error => expect(error).to.be.ok)
    })
  })
  ...
  describe('.destroy()', function() {
    ...
    it('should throw an error if the theatre has already been removed', async function() {
      ...
      return theatre.destroy().then(response => expect(response).to.not.be.ok).catch(error => expect(error).to.be.ok)
    })
  })
})
